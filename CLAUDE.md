...
- TypeScript 5.x, strict mode + VS Code Extension API, `IFileSystem` abstraction (`VSCodeFsAdapter` / `NodeFsAdapter`) — no new dependencies (011-unify-artifact-scanning)
- N/A — read-only filesystem scan of `.cursor/<type>/` and `.claude/<type>/` under each project root (011-unify-artifact-scanning)
