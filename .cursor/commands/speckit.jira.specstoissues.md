...
tools:
- '{mcp_server}/getAccessibleAtlassianResources'
...
**Resolve cloudId (do not store in config):**
- Do **not** add `cloudId` to jira-config.yml. The Atlassian MCP requires cloudId as a parameter for Jira tools; it is a site identifier, not a secret, but it must not be hardcoded.
- Before calling any Jira MCP tools, call **getAccessibleAtlassianResources** (no arguments). From the response array, take the resource whose `scopes` include Jira (e.g. `read:jira-work` or `write:jira-work`); use its **id** as `cloudId` for all subsequent calls (createJiraIssue, editJiraIssue, getJiraIssue, etc.).
- If multiple resources exist, prefer the one with Jira scopes. Use the same cloudId for the entire command run. This uses the current MCP session and requires no config.
...
Use the configured MCP server to create an Epic. Use the **cloudId** obtained in Step 2 (from getAccessibleAtlassianResources):

```
Tool: {mcp_server}/createJiraIssue
Parameters:
  - cloudId: {from getAccessibleAtlassianResources response, resource.id with Jira scopes}
...
**Note:** `cloudId` is not in config. It is resolved at runtime by calling `getAccessibleAtlassianResources` and using the resource `id` for the Jira site. Do not add cloudId to jira-config.yml.
