...
- Q: Should `.claude/agents/` at the project root use the same glob and display pattern as `.cursor/agents/`? → A: Yes — same flat `*.md` scan, same hubot icon, same alphabetical placement within the Claude section.
...
A developer opens a project whose workspace contains `.claude/agents/*.md` files. They expect to see those agent definitions in the Claude section of the Workspaces tree, the same way `.cursor/agents/*.md` files appear under the Cursor section.
...
**Independent Test**: Create `.claude/agents/my-agent.md` at a project root. Expand the project's Claude section in the Workspaces tree. Verify `my-agent.md` appears under an "Agents" subsection with the hubot icon and opens the file on click.

**Acceptance Scenarios**:

1. **Given** a project with one or more `.md` files in `.claude/agents/`, **When** I expand the Claude section in Workspaces, **Then** I see an "Agents" subsection (alphabetical among Claude's Rules, Commands, Skills, Agents) listing each file with the hubot icon.
...
3. **Given** a project with no `.claude/agents/` files (or directory absent), **When** I view the Claude section, **Then** the Agents subsection is absent or shows an explicit empty state — never misleading counts or errors.
4. **Given** a project with both `.cursor/agents/` and `.claude/agents/` files, **When** I expand Workspaces, **Then** both Cursor → Agents and Claude → Agents subsections are populated independently with no cross-contamination.

---

### User Story 2 — Cursor and Claude sections hidden when folder absent (Workspaces view) (Priority: P2)

A developer opens a project that uses only Claude Code (`.claude/` exists, `.cursor/` does not). They expect to see only a Claude section — not an empty Cursor section.
...
**Independent Test**: Open a workspace with `.claude/` present and no `.cursor/` directory. Verify the Workspaces tree shows a Claude section and no Cursor section.

**Acceptance Scenarios**:

1. **Given** a project with `.cursor/` present and `.claude/` absent, **When** I expand that project, **Then** I see the Cursor section and no Claude section.
2. **Given** a project with `.claude/` present and `.cursor/` absent, **When** I expand that project, **Then** I see the Claude section and no Cursor section.
...
- **FR-001**: The Workspaces tree MUST scan `.claude/agents/*.md` (flat, no recursion) at each project root and display the results in the Claude section under an "Agents" subsection, using the hubot icon for each leaf.
...
- **FR-005**: The file watcher for the project-level Claude section MUST include `.claude/agents/*.md` so the tree refreshes automatically when agent files are added, modified, or deleted (consistent with spec 006 FR-008 and spec 007 watcher patterns).
...
- **FR-007**: In the Workspaces tree, the Claude section for a project MUST be shown if and only if a `.claude/` directory exists at that project's root. If `.claude/` is absent, the Claude section MUST NOT appear.
...
- **FR-013**: The `providers` living spec MUST be updated to:
  - Add `'claude-agent-definitions'` and `'claude-agent-definition'` to the `category` union (both are subcategories of the existing `'claude-code'` category, which already exists in the codebase).
  - Update Regression Guardrail #5 to reflect that **platform sections** (Cursor, Claude) are hidden when their root folder is absent — only artifact-level nodes within a present platform use empty-state display.
- **FR-014**: The `tree-view` living spec MUST be updated to:
  - Add the Claude section to the Workspaces branch diagram and description, including Agents as a subsection.
...
- **SC-001**: A project with `.claude/agents/*.md` shows those files in the Workspaces Claude → Agents subsection; clicking one opens the file.
- **SC-002**: A project with only `.claude/` present shows no Cursor section in the Workspaces tree.
- **SC-003**: A project with only `.cursor/` present shows no Claude section in the Workspaces tree.
