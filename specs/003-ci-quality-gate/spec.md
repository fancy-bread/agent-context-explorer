...
As a maintainer, I want the test suite and instrumentation to bring **aggregate branch** coverage to at least **80%** (NYC **All files** row when `per-file` is false), so the branch gate is satisfied by design rather than by disabling checks.

**Why this priority**: The gate from User Story 1 only adds value once the codebase and tests actually meet the bar.

**Independent Test**: Run coverage; the **All files** row shows **branch** coverage **≥ 80%** (aggregate policy; see `contracts/ci-quality-gate.md`).

**Acceptance Scenarios**:

1. **Given** instrumented sources under the coverage policy, **When** coverage is generated after test work for this feature, **Then** aggregate branch coverage is **≥ 80%** (subject to existing exclude rules).

---

### User Story 3 - Tests meet function coverage bar (Priority: P3)

As a maintainer, I want the test suite and instrumentation to bring **aggregate function** coverage to at least **90%** (NYC **All files** row when `per-file` is false), so the function gate is satisfied consistently with branch and line gates.
...
**Independent Test**: Run coverage; the **All files** row shows **function** coverage **≥ 90%**.

**Acceptance Scenarios**:

1. **Given** instrumented sources under the coverage policy, **When** coverage is generated after test work for this feature, **Then** aggregate function coverage is **≥ 90%** (subject to existing exclude rules).
...
- **FR-001**: The **continuous integration** workflow for this repository MUST run the unit test suite with coverage and enforce **NYC `check-coverage`** (or equivalent) such that **branch** coverage below **80%** on the **aggregate** `All files` row (when `per-file` is **false** in `.nycrc`) causes a **non-zero** exit. If `per-file` is **true**, branch coverage below **80%** on any included file causes a **non-zero** exit.
- **FR-002**: The same pipeline MUST enforce **function** coverage such that **function** coverage below **90%** on the aggregate row (when `per-file` is **false**) causes a **non-zero** exit. If `per-file` is **true**, function coverage below **90%** on any included file causes a **non-zero** exit.
...
- **FR-005**: Coverage configuration MUST enforce **line** coverage on the same basis as FR-001/002 (aggregate when `per-file` is **false**). The repo targets **≥ 90%** lines on the aggregate row alongside **80%** branches and **90%** functions (see `contracts/ci-quality-gate.md`).
...
- **A-001**: The repository MAY use **`per-file`: false** so that **lines**, **branches**, and **functions** thresholds apply to the **All files** aggregate row only (see `contracts/ci-quality-gate.md`), trading strict per-file rows for a simpler gate while keeping strong overall coverage.
...
- **SC-002**: Measured **branch** coverage on successful runs is **≥ 80%** on the **All files** aggregate row when using **`per-file`: false** (or **≥ 80%** per file when `per-file` is **true**).
- **SC-003**: Measured **function** coverage on successful runs is **≥ 90%** on the aggregate row when using **`per-file`: false** (or **≥ 90%** per file when `per-file` is **true**).
- **SC-005**: Measured **line** coverage on successful runs is **≥ 90%** on the aggregate row when using **`per-file`: false** (aligned with `.nycrc` and `contracts/ci-quality-gate.md`).
